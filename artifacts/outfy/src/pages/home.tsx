    ...
    <div className="space-y-10">
      <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <div className="relative overflow-hidden rounded-[28px] bg-sidebar p-6 text-sidebar-foreground sm:p-9">
          ...
          <p className="relative mt-6 max-w-[440px] text-sm leading-relaxed text-sidebar-foreground/80">
            {t("hero.description")}
          </p>
          <div className="relative mt-8 flex flex-wrap gap-3">
            <Link
              href="/explore"
              className="outfy-primary-action inline-flex items-center gap-2 rounded-full bg-primary px-4 py-3 text-xs font-bold text-primary-foreground transition hover:-translate-y-0.5"
            ...
            <button
              type="button"
              onClick={onCreate}
              className="inline-flex items-center gap-2 rounded-full border border-primary/30 px-4 py-3 text-xs font-bold text-primary transition hover:bg-primary/10"
          ...
          {recommended.map((activity, index) => (
            ...
            >
              <ActivityCard
                ...
                interested={interested.has(activity.id)}
                onSave={onSave}
            ...
            {categories.map((category) => {
              ...
              return (
                <Link
                  ...
                  key={category}
                  className="flex items-center justify-between rounded-xl bg-card/70 px-3 py-2.5 text-xs font-semibold transition hover:bg-card"
                  data-testid={`link-category-${category}`}
                >
                  <span className="flex items-center gap-2">
                    <span
                      className={`h-2 w-2 rounded-full tone-dot-${meta.tone}`}
                    />
                    {meta.label}
                  </span>
